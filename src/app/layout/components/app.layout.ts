...
import { AppRightDrawer } from './app.rightdrawer';

@Component({
    selector: 'app-layout',
    standalone: true,
    imports: [CommonModule, AppTopbar, AppSidebar, RouterModule, AppFooter, AppConfigurator, AppBreadcrumb, AppRightDrawer],
    template: `
...
export class AppLayout implements OnDestroy {
    ...
    handleRouteChange(url: string): void {
        // Parse URL to detect if on member page
        // Example: /pages/member/MEM-001234 or /pages/member/MEM-001234/claims
        const memberMatch = url.match(/\/pages\/member\/([^\/]+)/);
        if (memberMatch) {
            const memberId = memberMatch[1];
            this.layoutService.setActiveMemberMenu(memberId);
        } else {
            // Not on a member page, clear active member menu
            this.layoutService.setActiveMemberMenu(null);
        }
    }
