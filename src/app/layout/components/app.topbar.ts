    ...
    template: `
        <div class="layout-topbar">
            <a class="app-logo" routerLink="/">
                ...
                <span class="app-name">Ventana</span>
            </a>
            ...
            <div class="topbar-actions">
                ...
                <div class="topbar-profile">
                    <button class="topbar-profile-button" type="button" pStyleClass="@next" enterFromClass="hidden" enterActiveClass="animate-scalein" leaveToClass="hidden" leaveActiveClass="animate-fadeout" [hideOnOutsideClick]="true">
                        <img alt="avatar" src="/layout/images/prince.png" />
                        <span class="profile-details">
                            <span class="profile-name">Prince Rogers Nelson</span>
                    ...
                    <ul class="list-none hidden p-2 sm:p-4 m-0 rounded-border shadow absolute bg-surface-0 dark:bg-surface-900 origin-top w-48 mt-2 right-0 top-auto">
                        <li class="flex items-start flex-col">
                            ...
                            <a pRipple class="flex p-2 rounded-border w-full items-center hover:bg-emphasis transition-colors duration-150 cursor-pointer" (click)="layoutService.showConfigSidebar()">
                                <i class="pi pi-cog mr-4"></i>
                                <span>Settings</span>
                            </a>
                            <a pRipple class="flex p-2 rounded-border w-full items-center hover:bg-emphasis transition-colors duration-150 cursor-pointer" (click)="openRightDrawer()">
                                <i class="pi pi-palette mr-4"></i>
...
export class AppTopbar {
    ...
    removeTab(event: MouseEvent, item: MenuItem, index: number) {
        // Extract member ID from route if it's a member tab
        if (item.routerLink && item.routerLink.length >= 2 && item.routerLink[0] === '/pages/member') {
            const memberId = item.routerLink[1];
            this.layoutService.removeMemberMenu(memberId);
        }
